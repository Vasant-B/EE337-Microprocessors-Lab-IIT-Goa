...
RET		
;---------------------------------------------------------------
LCD_DISPLAY_REGISTER:
	PUSH AR0
; First configure switches as input and LEDs as output
	MOV A, #0FH
	MOV P1, A 
; To configure a port as output clear it 
; To configure a port as input set it ;initialise LCD

; Display "Enter memory location" on the LCD
	
	MOV A, #80H			;Put cursor on first row,1 column
	LCALL LCD_COMMAND
	LCALL DELAY
	MOV DPTR, #MY_STRING_ENTER_MEMORY
	LCALL LCD_SENDSTRING
	LCALL DELAY
	LCALL DELAY
	LCALL DELAY
...
; Wait for 5 seconds
	LCALL DELAY_5s 
; Read 8 bit value using subroutine PACKNIBBLES
	LCALL PACKNIBBLES 
; Display the value of memory address and content in Hex on LCD
	MOV R0, 4FH
; e.g.: Display "12 53" when the memory location 12H contains value 53H
	MOV A, #80H			;Put cursor on first row,1 column
	LCALL LCD_COMMAND
	LCALL DELAY
	...
	;Put the cursor on the first row at 0th coloumn
	MOV A, #80H			;Put cursor on first row,1 column
	LCALL LCD_COMMAND
	LCALL DELAY
	;Display the location (first convert to ASCII)
	MOV A, R0
	LCALL ASCIICONV
	;First, display Higher Nibble
	LCALL LCD_SENDDATA 
	...
	;Display the content pointed by the location (first convert to ASCII)
	MOV A, @R0
	LCALL ASCIICONV
	;First, display Higher Nibble
	LCALL LCD_SENDDATA 
